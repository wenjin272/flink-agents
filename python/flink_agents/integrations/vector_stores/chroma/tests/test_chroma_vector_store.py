...
def test_translate_filters_single_key_pass_through() -> None:
    # Shorthand equality and operator form are native ChromaDB — no rewriting.
    assert _translate_filters_to_chroma_where({"k": "v"}) == {"k": "v"}
    assert _translate_filters_to_chroma_where({"n": {"$gt": 1}}) == {"n": {"$gt": 1}}
...
def test_translate_filters_connectives_pass_through() -> None:
    # Explicit $and / $or are already native.
    assert _translate_filters_to_chroma_where({"$and": [{"k": "v"}, {"n": 1}]}) == {
        "$and": [{"k": "v"}, {"n": 1}]
    }
    assert _translate_filters_to_chroma_where({"$or": [{"k": "v"}, {"k": "x"}]}) == {
        "$or": [{"k": "v"}, {"k": "x"}]
    }


def test_translate_filters_top_level_not_rejected() -> None:
    with pytest.raises(NotImplementedError, match=r"\$not"):
        _translate_filters_to_chroma_where({"$not": {"k": "v"}})


def test_translate_filters_nested_not_rejected() -> None:
    # $not anywhere in the tree — in a branch of $or, inside a value dict —
    # must be caught before hitting ChromaDB.
    with pytest.raises(NotImplementedError, match=r"\$not"):
        _translate_filters_to_chroma_where({"$or": [{"$not": {"k": "v"}}, {"n": 1}]})
    with pytest.raises(NotImplementedError, match=r"\$not"):
        _translate_filters_to_chroma_where({"$and": [{"k": "v"}, {"$not": {"n": 1}}]})
...
@pytest.mark.skipif(not chromadb_available, reason="ChromaDB is not available")
def test_query_with_operator_filter() -> None:
    vs = _filter_test_store()
    result = vs.query(
        VectorStoreQuery(
            query_text="x",
...
@pytest.mark.skipif(not chromadb_available, reason="ChromaDB is not available")
def test_query_with_or_filter() -> None:
    vs = _filter_test_store()
    result = vs.query(
        VectorStoreQuery(
            query_text="x",
            limit=5,
            collection_name="filter_tests",
            filters={"$or": [{"category": "b"}, {"score": 10}]},
        )
    )
    assert sorted(d.id for d in result.documents) == ["doc1", "doc3"]


@pytest.mark.skipif(not chromadb_available, reason="ChromaDB is not available")
def test_query_with_in_filter() -> None:
    vs = _filter_test_store()
    result = vs.query(
        VectorStoreQuery(
            query_text="x",
...
@pytest.mark.skipif(not chromadb_available, reason="ChromaDB is not available")
def test_query_raises_on_not_operator() -> None:
    vs = _filter_test_store()
    with pytest.raises(NotImplementedError, match=r"\$not"):
        vs.query(
            VectorStoreQuery(
