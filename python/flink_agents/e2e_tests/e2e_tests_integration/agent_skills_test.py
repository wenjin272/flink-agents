...
class SkillTestAgent(Agent):
    ...
    def ollama_connection() -> ResourceDescriptor:
        return ResourceDescriptor(
            clazz=ResourceName.ChatModel.OLLAMA_CONNECTION, request_timeout=300
        )

    @chat_model_setup
    @staticmethod
    def ollama_setup() -> ResourceDescriptor:
        return ResourceDescriptor(
    ...
    def my_skills() -> Skills:
        return Skills(paths=[str(current_dir.parent / "resources" / "skills")], allowed_commands=["echo", "bc"])
...
@pytest.mark.skip(reason="Without GPU support, model inference takes too long.")
...
def test_react_agent_with_skills(tmp_path: Path) -> None:
    ...
    (
        env.add_resource(
            ...
            "my_skill",
            ResourceType.SKILL,
            Skills(paths=[str(current_dir.parent / "resources" / "skills")],
