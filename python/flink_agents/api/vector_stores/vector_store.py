...
class VectorStoreQuery(BaseModel):
    ...
    filters: Dict[str, Any] | None = Field(
        default=None,
        description=(
            "Metadata filter in the flink-agents unified filter DSL. Each "
            "vector store translates this to its own native dialect. See "
            ":class:`BaseVectorStore` for the DSL grammar."
        ),
    )
...
class BaseVectorStore(Resource, ABC):
    ...
    unified, MongoDB-style dialect — implementations translate to their
    native filter format internally. Callers never need to know the native
    dialect.

    Grammar (values can be any JSON-compatible scalar / list)::

        # Equality shorthand — "field equals value":
        {"field": value}
        ...
        # is equivalent to
        {"$and": [{"user_id": "u1"}, {"run_id": "r1"}]}

    ``None`` means "no filter". Stores that lack native support for a
    given operator should raise ``NotImplementedError`` with a clear
    message rather than silently drop the clause.
    """
