...
public class CompileUtilsTest {
    ...
    // Workflow logic: x -> (x + 1) * 2
    private static final WorkflowPlan TEST_WORKFLOW_PLAN =
            ActionExecutionOperatorTest.TestWorkflow.getWorkflowPlan();

    @Test
    void testJavaNoKeyedStreamConnectToWorkflow() throws Exception {
        ...
        DataStream<Object> workflowOutputStream =
                CompileUtils.connectToWorkflow(
                        inputStream,
    ...
    @Test
    void testKeyedStreamConnectToWorkflow() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

        KeyedStream<Long, Long> keyedInputStream =
                env.fromSequence(TEST_SEQUENCE_START, TEST_SEQUENCE_END).keyBy(x -> x);
        DataStream<Object> workflowOutputStream =
                CompileUtils.connectToWorkflow(keyedInputStream, TEST_WORKFLOW_PLAN);
        DataStream<Long> resultStream = workflowOutputStream.map(x -> (long) x + 1);
